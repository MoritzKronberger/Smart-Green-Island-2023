...
from app.settings import ARUCO_DICT, CAMERA, MOCK_IMAGE_PATH, PERSPECTIVE_CORRECTION_MARKER_ID
...
def __loop() -> None:
    ...
    # Detect marker
    marker = aruco.detect_marker(image, PERSPECTIVE_CORRECTION_MARKER_ID)
    if marker:
        # Render marker
        marker.visualize(image)
        # Update the camera's perspective correction
        camera.perspective_transform_matrix = marker.get_perspective_transform_matrix()
